 /// result current uder review in the top level
...
//   const { reviewee_Id, customerId, reviewId } = req.body; // Use req.query for GET requests
...
const getRatings = async (req, res) => {
  const { reviewee_Id } = req.query; // Using query parameters for GET request

  if (!reviewee_Id) {
    return sendGeneralResponse(res, false, "reviewee_Id is required", 400);
  }
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      ratings: allRatings // All ratings sorted by latest first
    });
