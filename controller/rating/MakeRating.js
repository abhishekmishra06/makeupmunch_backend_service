...
// allow one user make multiple ratings
...
const makeRating = async (req, res) => {
  ...
  // Validate required fields: Ensure customerId and reviewee_Id are provided
  if (!customerId || !reviewee_Id) {
    return sendGeneralResponse(res, false, "Customer ID and Artist ID are required", 400);
  }
  ...
  try {
    ...
    const newRating = new Rating({
      customerId,
      reviewee_Id,
      rating: rating || null, // If rating is provided, save it; otherwise, save null
      message: message || '', // If message is provided, save it; otherwise, save empty string
    });
